...
class Solution:
    def mincostToHireWorkers(self, quality: List[int], wage: List[int], k: int) -> float:
        ...
        #       iterate through the workers by the wage/quality ratio
        #       maintain a list of k workers with the highest quality so far
        #       compute the cost of that group by multiplying their summed quality with the largest wage/quality ratio in the group
        ...
        for i in range(len(quality)):
            heappush(maxheap, -ratio[i][1])  # quality values are negative to give us a max heap
            sum_quality += ratio[i][1]
            while len(maxheap) > k:
                sum_quality += heappop(maxheap)  # substract from the total quality from the group
            if len(maxheap) == k:
